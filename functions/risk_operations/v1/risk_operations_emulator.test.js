...
async function main() {
  try {
    ...
    const result = await handler({auth: {uid}, app: {appId: "emulator-app-check"}, data: {pageSize: 10}});
    const after = await state();
    assert.equal(result.readOnly, true);
    assert.equal(result.writesPerformed, 0);
    assert.equal(result.items.length, 1);
    assert.deepEqual(after, before);
    await assert.rejects(() => handler({auth: {uid}, app: {appId: "emulator-app-check"}, data: {tenantId: "other"}}), /mismatch/);
