...
function diagnosticLogFields(diagnostics, revision = process.env.K_REVISION || "unknown") {
  return Object.freeze({hashedClientTabId: hashDiagnosticId(diagnostics.clientTabId), hashedNavigationId: hashDiagnosticId(diagnostics.navigationId), hashedPageInstanceId: hashDiagnosticId(diagnostics.pageInstanceId), hashedLoadAttemptId: hashDiagnosticId(diagnostics.loadAttemptId), trigger: diagnostics.trigger, attemptSequence: diagnostics.attemptSequence, revision, adapterVersion: ADAPTER_VERSION});
}
