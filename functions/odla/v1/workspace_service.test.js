...
function adapter(overrides={}) {
  ...
  return {
    ...
    createCase: async (args)=>{
      calls.push(["createCase", args]); return {data: args.caseRecord, idempotent: false};
    },
    ...
    appendCustodyEvent: async (args)=>{
      calls.push(["appendCustodyEvent", args]); return {data: args.event, idempotent: false};
    },
    getLaboratory: async (id)=>({...lab, laboratoryId: id}),
    createTestRequest: async (args)=>{
      calls.push(["createTestRequest", args]); return {data: args.requestRecord, idempotent: false};
    },
    getTestRequest: async (_caseId, id)=>defaultRequest(id),
    createTestResult: async (args)=>{
      calls.push(["createTestResult", args]); return {data: args.resultRecord, idempotent: false};
    },
    writeFinding: async (args)=>{
      calls.push(["writeFinding", args]); return {data: args.findingRecord, idempotent: false};
    },
    getFinding: async (_caseId, id)=>({findingId: id, primaryLaboratoryId: "l1"}),
    createAppeal: async (args)=>{
      calls.push(["createAppeal", args]); return {data: args.appealRecord, idempotent: false};
    },
    ...
    resolveAppeal: async (args)=>{
      calls.push(["resolveAppeal", args]); return {data: args.resolution, idempotent: false};
    },
...
test("ODLA-SEC-SVC-002 client jurisdictionOverride is never authoritative", async () => {
  ...
  const s=createOdlaWorkspaceService({adapter: adapter({
    createCase: async ({caseRecord})=>{
      captured=caseRecord; return {data: caseRecord, idempotent: false};
    },
  })});
...
test("ODLA-SEC-SVC-009 persisted open material appeal blocks permanent finality", async () => {
  ...
  const s=createOdlaWorkspaceService({adapter: adapter({
    ...
    writeFinding: async ({findingRecord})=>{
      captured=findingRecord; return {data: findingRecord};
    },
  })});
  await s.adjudicateOdlaFinding({authority: authority("authorized_reviewer"), data: {
...
test("ODLA-SEC-SVC-014 appeal stores original primary laboratory only from persisted challenged finding", async () => {
  ...
  const s=createOdlaWorkspaceService({adapter: adapter({
    ...
    createAppeal: async ({appealRecord})=>{
      captured=appealRecord; return {data: appealRecord};
    },
  })});
...
test("ODLA-SEC-SVC-016 appeal resolution uses persisted appeal and persisted appeal test request identities", async () => {
  ...
  const s=createOdlaWorkspaceService({adapter: adapter({
    resolveAppeal: async (args)=>{
      captured=args; return {data: args.resolution};
    },
  })});
