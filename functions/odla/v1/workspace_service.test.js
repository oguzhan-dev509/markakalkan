...
function authority(role = "verified_rightsholder") {
  return {
    ...
    tenantId: "t1",
    brandUids: ["b1"],
    assignedLaboratoryIds: ["l1", "l2"],
  };
}
const ov = {
  status: "active",
  countryCode: "TR",
  productClassCode: "cosmetic",
  overrideVersion: 1,
};
const profile = {
  profileId: "p1",
  status: "active",
  profileCode: "COSMETIC_V1",
  profileVersion: 1,
  productClassCode: "cosmetic",
  jurisdictionOverrides: {TR: ov},
};
const baseCase = {
  caseId: "c1",
  tenantId: "t1",
...
const lab = {
  laboratoryId: "l1",
  status: "active",
  productClassCodes: ["cosmetic"],
  testMethodCodes: ["METHOD1"],
  scopeCodes: ["SCOPE1"],
...
function adapter(overrides = {}) {
  ...
  return {
    calls,
    createCase: async (args) => {
      calls.push(["createCase", args]);
    ...
    appendCustodyEvent: async (args) => {
      calls.push(["appendCustodyEvent", args]);
      return {data: args.event, idempotent: false};
    },
    getLaboratory: async (id) => ({...lab, laboratoryId: id}),
    createTestRequest: async (args) => {
      calls.push(["createTestRequest", args]);
      return {data: args.requestRecord, idempotent: false};
    },
    getTestRequest: async (_caseId, id) => defaultRequest(id),
    createTestResult: async (args) => {
      calls.push(["createTestResult", args]);
      return {data: args.resultRecord, idempotent: false};
    },
    writeFinding: async (args) => {
      calls.push(["writeFinding", args]);
      return {data: args.findingRecord, idempotent: false};
    },
    getFinding: async (_caseId, id) => ({
      findingId: id,
      primaryLaboratoryId: "l1",
    }),
    createAppeal: async (args) => {
      calls.push(["createAppeal", args]);
      return {data: args.appealRecord, idempotent: false};
    },
    getAppeal: async (_caseId, id) => ({
      appealId: id,
      challengedFindingId: "f1",
      originalPrimaryLaboratoryId: "l1",
      appealTestRequestId: "q2",
      state: "opened",
      material: true,
    }),
    resolveAppeal: async (args) => {
      calls.push(["resolveAppeal", args]);
      return {data: args.resolution, idempotent: false};
    },
    getAdjudicationContext: async () => ({
      openMaterialAppeal: false,
      unresolvedIntegrityConflict: false,
      missingRequiredEvidence: false,
      singleReporterOnly: false,
      sellerSuppliedSampleSole: false,
...
function caseData() {
  return {
    ...
    profileId: "p1",
    countryCode: "TR",
    profileCode: "CLIENT_IGNORED",
    productClassCode: "CLIENT_IGNORED",
...
function appealOpenData() {
  return {
    ...
    appealId: "a1",
    challengedFindingId: "f1",
    groundCode: "request_second_independent_lab",
  };
}
function appealResolveData() {
  return {
    ...
    caseId: "c1",
    appealId: "a1",
    conflictingLaboratoryResults: false,
    findingInputs: {rightsholderConfirmed: true},
  };
}
...
test("ODLA-BE-SVC-001 create case requires authorized requester", async () => {
  ...
  await assert.rejects(() =>
    s.createOdlaVerificationCase({
      authority: authority("assigned_laboratory"),
      data: caseData(),
    }),
  );
});
...
test(
    "ODLA-BE-SVC-002 workspace read returns " + "authorization-scoped DTO",
    async () => {
      const s = createOdlaWorkspaceService({adapter: adapter()});
      assert.equal(
          (
            await s.getOdlaVerificationWorkspace({
              authority: authority(),
...
test("ODLA-BE-SVC-003 custody sequence gap is rejected", async () => {
  const prev = {
    ...
    appendOnly: true,
    eventId: "x",
    eventPayloadSha256: "y",
  };
  const s = createOdlaWorkspaceService({
    adapter: adapter({getLatestCustodyEvent: async () => prev}),
  });
  await assert.rejects(() =>
    s.appendOdlaChainOfCustodyEvent({
      authority: authority(),
      data: {
        operationId: "op2",
        tenantId: "t1",
...
test("ODLA-BE-SVC-004 custody previous-hash mismatch is rejected", async () => {
  ...
  const prev = core.buildCustodyEvent({
    tenantId: "t1",
    brandUid: "b1",
    caseId: "c1",
    sampleId: "s1",
  ...
  const s = createOdlaWorkspaceService({
    adapter: adapter({getLatestCustodyEvent: async () => tampered}),
  });
  await assert.rejects(() =>
    s.appendOdlaChainOfCustodyEvent({
      authority: authority(),
...
test(
    "ODLA-BE-SVC-005 seal continuity failure " +
    "is denied or explicitly escalated",
    async () => {
      const core = require("../chain_of_custody");
      const prev = core.buildCustodyEvent({
        tenantId: "t1",
...
test(
    "ODLA-BE-SVC-006 lab product-method-" + "scope mismatch is rejected",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
...
test(
    "ODLA-BE-SVC-008 seller-supplied sample " +
    "cannot be sole final evidence",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          getAdjudicationContext: async () => ({
            ...
            missingRequiredEvidence: false,
            singleReporterOnly: false,
            sellerSuppliedSampleSole: true,
          }),
        }),
      });
      await assert.rejects(() =>
        s.adjudicateOdlaFinding({
...
test(
    "ODLA-BE-SVC-009 brand-owned lab " +
    "evidence remains explicitly classified",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          ...
          createTestRequest: async ({requestRecord}) => {
            ...
            return {data: requestRecord, idempotent: false};
          },
        }),
      });
      await s.createOdlaTestRequest({
        authority: authority(),
        data: testReqData(),
      });
...
test(
    "ODLA-BE-SVC-010 quality nonconformance " +
    "alone cannot become counterfeit",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          writeFinding: async ({findingRecord}) => {
            captured = findingRecord;
            return {data: findingRecord, idempotent: false};
          },
        }),
      });
      await s.adjudicateOdlaFinding({
        ...
        data: {
          ...findingData(),
          findingInputs: {qualityNonconforming: true},
        },
      });
...
test(
    "ODLA-BE-SVC-011 single reporter cannot " +
    "create final counterfeit finding",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          getAdjudicationContext: async () => ({
...
test(
    "ODLA-BE-SVC-012 open material appeal " +
    "blocks permanent adverse finality",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
...
test(
    "ODLA-BE-SVC-013 appeal cannot reuse " + "original primary laboratory",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          getAppeal: async () => ({
...
test(
    "ODLA-BE-SVC-014 conflicting laboratory " +
    "results enter conflict review",
    async () => {
      const s = createOdlaWorkspaceService({adapter: adapter()});
...
test(
    "ODLA-SEC-SVC-001 case creation requires " + "server-loaded profileId",
    async () => {
      let seen = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          getVerificationProfile: async (id) => {
            seen = id;
            return {...profile};
          },
        }),
      });
      await s.createOdlaVerificationCase({
        authority: authority(),
        data: caseData(),
      });
...
test(
    "ODLA-SEC-SVC-002 client " +
    "jurisdictionOverride is never " +
    "authoritative",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          createCase: async ({caseRecord}) => {
            captured = caseRecord;
            return {data: caseRecord, idempotent: false};
          },
        }),
      });
      await s.createOdlaVerificationCase({
...
test(
    "ODLA-SEC-SVC-003 case profile code " +
    "version and product class derive from " +
    "trusted profile document",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
...
test(
    "ODLA-SEC-SVC-004 test request policy " +
    "derives from persisted case plus " +
    "trusted profile",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
...
test(
    "ODLA-SEC-SVC-005 result for nonexistent " +
    "persisted test request is rejected",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          getTestRequest: async () => {
...
test(
    "ODLA-SEC-SVC-006 result sampleId " +
    "mismatch against persisted test request " +
    "is rejected",
    async () => {
      const s = createOdlaWorkspaceService({adapter: adapter()});
      await assert.rejects(
          () =>
            s.recordOdlaTestResult({
              authority: authority("assigned_laboratory"),
              data: {...resultData(), sampleId: "wrong"},
            }),
...
test(
    "ODLA-SEC-SVC-007 result laboratoryId " +
    "mismatch against persisted test request " +
    "is rejected",
    async () => {
      const s = createOdlaWorkspaceService({adapter: adapter()});
      await assert.rejects(
          () =>
            s.recordOdlaTestResult({
              authority: authority("assigned_laboratory"),
              data: {...resultData(), laboratoryId: "l2"},
            }),
...
test(
    "ODLA-SEC-SVC-008 assigned-laboratory " +
    "authorization uses persisted request " +
    "laboratoryId",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
      ...
      const a = {
        ...authority("assigned_laboratory"),
        assignedLaboratoryIds: ["l1"],
      };
      await assert.rejects(() =>
        s.recordOdlaTestResult({
          authority: a,
          data: {...resultData(), laboratoryId: undefined},
        }),
      );
    },
);
...
test(
    "ODLA-SEC-SVC-009 persisted open " +
    "material appeal blocks permanent " +
    "finality",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
...
test(
    "ODLA-SEC-SVC-010 missing trusted " +
    "evidence summary blocks permanent " +
    "finality",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
...
test(
    "ODLA-SEC-SVC-011 persisted single-" +
    "reporter-only evidence blocks permanent " +
    "finality",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
...
test(
    "ODLA-SEC-SVC-012 persisted seller-" +
    "supplied-sole-sample evidence blocks " +
    "permanent finality",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
      ...
      await assert.rejects(() =>
        s.adjudicateOdlaFinding({
          ...
          data: {
            ...findingData(),
            evidence: {sellerSuppliedSampleSole: false},
          },
        }),
      );
    },
...
test(
    "ODLA-SEC-SVC-013 opening appeal against " +
    "nonexistent challenged finding is " +
    "rejected",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
      ...
      await assert.rejects(() =>
        s.openOdlaAppeal({authority: authority(), data: appealOpenData()}),
      );
    },
);

test(
    "ODLA-SEC-SVC-014 appeal stores original " +
    "primary laboratory only from persisted " +
    "challenged finding",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          getFinding: async () => ({
            findingId: "f1",
            primaryLaboratoryId: "SERVER-LAB",
          }),
          createAppeal: async ({appealRecord}) => {
            captured = appealRecord;
            return {data: appealRecord};
          },
        }),
      });
      await s.openOdlaAppeal({
        authority: authority(),
        data: {
          ...appealOpenData(),
          originalPrimaryLaboratoryId: "CLIENT-LAB",
        },
      });
...
test(
    "ODLA-SEC-SVC-015 appeal-linked test " +
    "request uses persisted appeal primary " +
    "lab and rejects same lab",
    async () => {
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          getAppeal: async () => ({
...
test(
    "ODLA-SEC-SVC-016 appeal resolution uses " +
    "persisted appeal and persisted appeal " +
    "test request identities",
    async () => {
      let captured = null;
      const s = createOdlaWorkspaceService({
        adapter: adapter({
          resolveAppeal: async (args) => {
            captured = args;
            return {data: args.resolution};
          },
        }),
      });
      await s.resolveOdlaAppeal({
        authority: authority("authorized_reviewer"),
        data: {
          ...appealResolveData(),
          originalPrimaryLaboratoryId: "CLIENT-A",
...
test(
    "ODLA-SEC-SVC-017 case creation passes " +
    "server-generated audit context to " +
    "adapter",
    async () => {
      const a = adapter();
      ...
      assert.equal(ctx.source, "odla_server_trusted_context");
    },
);

test(
    "ODLA-SEC-SVC-018 custody append passes " +
    "server-generated audit context to " +
    "adapter",
    async () => {
      const a = adapter();
      ...
      assert.equal(
          a.calls.find((x) => x[0] === "appendCustodyEvent")[1].auditContext
              .action,
          "append_custody",
      );
    },
...
test(
    "ODLA-SEC-SVC-019 test request creation " +
    "passes server-generated audit context " +
    "to adapter",
    async () => {
      const a = adapter();
      const s = createOdlaWorkspaceService({adapter: a});
      ...
      assert.equal(
          a.calls.find((x) => x[0] === "createTestRequest")[1].auditContext
              .action,
          "create_test_request",
      );
    },
...
test(
    "ODLA-SEC-SVC-020 result recording " +
    "passes server-generated audit context " +
    "to adapter",
    async () => {
      const a = adapter();
      const s = createOdlaWorkspaceService({adapter: a});
      await s.recordOdlaTestResult({
        authority: authority("assigned_laboratory"),
        data: resultData(),
      });
      assert.equal(
          a.calls.find((x) => x[0] === "createTestResult")[1].auditContext
              .action,
          "record_test_result",
      );
    },
...
test(
    "ODLA-SEC-SVC-021 finding adjudication " +
    "passes server-generated audit context " +
    "to adapter",
    async () => {
      const a = adapter();
      ...
      assert.equal(
          a.calls.find((x) => x[0] === "writeFinding")[1].auditContext.action,
          "adjudicate_finding",
      );
    },
);
...
test(
    "ODLA-SEC-SVC-022 appeal opening passes " +
    "server-generated audit context to " +
    "adapter",
    async () => {
      const a = adapter();
      ...
      assert.equal(
          a.calls.find((x) => x[0] === "createAppeal")[1].auditContext.action,
          "open_appeal",
      );
    },
);
...
test(
    "ODLA-SEC-SVC-023 appeal resolution " +
    "passes server-generated audit context " +
    "to adapter",
    async () => {
      const a = adapter();
      ...
      assert.equal(
          a.calls.find((x) => x[0] === "resolveAppeal")[1].auditContext.action,
          "resolve_appeal",
      );
    },
);
...
test(
    "ODLA-SEC-SVC-024 client-supplied audit " +
    "or trust fields cannot override server-" +
    "generated audit context",
    async () => {
      const a = adapter();
      const s = createOdlaWorkspaceService({adapter: a});
      await s.createOdlaVerificationCase({
