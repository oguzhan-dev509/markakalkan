...
test('ADV-004 payer pressure cannot mutate evidence',()=>{const x=c.validateTestResult({testResultId:'r',testRequestId:'q',sampleId:'s',laboratoryId:'l',laboratoryReportId:'rep',reportSha256:'a'.repeat(64),appendOnly:true});assert.equal(Object.isFrozen(x),true)});
...
test('ADV-030 same identity different payload conflicts conceptually',()=>{assert.equal(c.deriveDeterministicId('op',['x']),c.deriveDeterministicId('op',['x']));assert.notEqual(c.canonicalJson({x:1}),c.canonicalJson({x:2}))});
